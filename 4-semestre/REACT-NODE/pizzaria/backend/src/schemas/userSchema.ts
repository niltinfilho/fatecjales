...
export const createUserSchema = z.object({
  ...
  // Define o que será validado o corpo (body) da requisição
  body: z.object({

    // Validação do campo "name"
    name: z
      // Define que deve ser uma String (text)
      .string({ message: "O nome precisa ser um texto" })
      // Define que deve no mínimo 3 caracteres
...
// Cria um schema de validação para login de usuário
export const authUserSchema = z.object({
  body: z.object({
    // Define que deve ser uma string no formato de e-mail válido
    email: z.email({ message: "Precisa ser um e-mail válido!" }),
    password: z.
      string({ message: "A senha é obrigatória!" })
      // Define que deve ter no mínimo 1 caracterer
